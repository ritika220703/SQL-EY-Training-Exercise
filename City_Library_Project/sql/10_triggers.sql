...
FOR EACH ROW
BEGIN
  INSERT INTO audit_log (table_name, action, record_id, description, action_time)
  VALUES (
    'members',
    'UPDATE',
    NEW.member_id,
...
FOR EACH ROW
BEGIN
  -- Automatically set due_date to 14 days after loan_date if NULL
...
BEGIN
  ...
  -- Only when status changes from 'active' → 'returned'
